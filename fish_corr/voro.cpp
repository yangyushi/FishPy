...
vector<int> Fish_tank::indices_with_volume(py::array_t<int> indices){
    c_loop_all particle_iter(this->core);
    voronoicell cell;
    int particle_id;
    double x, y, z, radius;
...
py::array_t<int> get_indices_with_voro_volumes_select(py::array_t<double> points, py::array_t<int> indices, double z, double c){
    // get indices that has voronoi volumes of selected points
    ...
    vector<int> indices_with_volume = tank.indices_with_volume(indices);

    py::array_t<int> indices_array = py::array_t<int>(indices_with_volume.size());

    auto buf_idx = indices_array.request();
    int * ptr_idx = (int *) buf_idx.ptr;

    for (size_t i = 0; i < indices_with_volume.size(); i++) {
        ptr_idx[i] = indices_with_volume[i];
    }
...
py::array_t<double> get_voro_volumes_select(py::array_t<double> points, py::array_t<int> indices, double z, double c){
    ...
    py::array_t<double> volume_array = py::array_t<double>(volumes.size());

    auto buf_vol = volume_array.request();
    double * ptr_vol = (double *) buf_vol.ptr;
...
PYBIND11_MODULE(voro, m) {
    ...
    m.def("get_indices_with_voro_volumes_select", &get_indices_with_voro_volumes_select,
