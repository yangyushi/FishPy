...
def get_clusters(feature, kernels, angles, kernel_threshold=0.0):
    """
    Return:
        list: the pixels that 'belong' to different features
...
def get_clusters_with_roi(feature, kernels, angles, roi, kernel_threshold=0.0):
    """
    Return:
        list: the pixels that 'belong' to different features
              the returnned results are (x, y), the shape is (number, dimension)
...
def get_clusters_from_image(feature, kernels, image, angles, roi, kernel_threshold=0.0) -> list:
    """
    Args:
        feature (np.ndarray): oishi feature generated by :meth:`get_oishi_features`
        kernels (:obj:`list` of np.ndarray): oishi kernels generated by :meth:`get_oishi_kernels`
...
def o2v(orientation, angles) -> np.ndarray:
    ...
    Args:
        orientation (int): label of the orientation.
                           ``angles[labels]`` yields the actural angle value
        angles (np.ndarray): the unit is degree

    Return:
        np.ndarray: the ``(x, y)`` direction of the orientation
    """
...
def get_oishi_features(image, oishi_kernels, threshold=0.5, local_size=4):
    ...
    Args:
        ...
        oishi_kernels (:obj:`list` of np.ndarray): the kernels with different shapes/scales/orientations
        threshold (float): intensity threshold
        local_size (int):  the local maxima defined were considered as potential oishi features

    Return:
        np.ndarray: oishi features, shape (6, n)
              [x, y, orientation, shape, brightness, likelihood]
              the returned feature location is ``(x, y)`` in the image coordinate
              NOT ``(row, colume)``
    """
...
def get_align_diff(orientations):
    ...
    Return:
        np.ndarray: the distance between orientations
    """
